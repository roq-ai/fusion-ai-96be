...
import { getFriends } from 'apiSdk/friends';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const { data, error, isLoading } = useSWR<FriendInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/friends` : null),
    () => getFriends({ tenant_id: session?.user?.tenantId }).then(({ data }) => data),
  );
  loading = isLoading;
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    {
      name: 'Friends',
      path: isTenantUser ? `/friends/view/${data?.[0]?.id}` : '/friends',
      entity: 'friend',
